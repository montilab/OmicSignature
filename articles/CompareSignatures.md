...
\
[`data`](https://rdrr.io/r/utils/data.html)`(``compare_signatures_example``)`\
`signature_list`` ``<-`` ``compare_signatures_example`\
\
`overlap_res`` ``<-`` `[`compare_omic_signatures`](https://montilab.github.io/OmicSignature/reference/compare_omic_signatures.md)`(`\
`  sig_list1 ``=`` ``signature_list``,`\
`  method ``=`` ``"overlap"``,`\
`  score_cutoff ``=`` `[`log2`](https://rdrr.io/r/base/Log.html)`(``1.25``)``,`\
`  adj_p_cutoff ``=`` ``0.05``,`\
`  min_features ``=`` ``25``,`\
`  max_feature ``=`` ``2000`\
`)`\
...
[`names`](https://rdrr.io/r/base/names.html)`(``overlap_res``$``comparisons``)`\
...
\
`cross_res`` ``<-`` `[`compare_omic_signatures`](https://montilab.github.io/OmicSignature/reference/compare_omic_signatures.md)`(`\
...
\
[`data`](https://rdrr.io/r/utils/data.html)`(``compare_label_pairing_example``)`\
\
`## let's see the order of the signatures' group_label's`\
`compare_label_pairing_example`` ``|>`\
`  `[`sapply`](https://rdrr.io/r/base/lapply.html)`(``\``(``x``)`` `[`levels`](https://rdrr.io/r/base/levels.html)`(``x``$``difexp``$``group_label``)``)`` ``|>`` `[`t`](https://rdrr.io/r/base/t.html)`(``)`` ``|>`\
`  `[`as.data.frame`](https://rdrr.io/r/base/as.data.frame.html)`(``)`` ``|>`` `[`setNames`](https://rdrr.io/r/stats/setNames.html)`(`[`c`](https://rdrr.io/r/base/c.html)`(``"level1"``, ``"level2"``)``)`\
`#>                level1    level2`\
...
`paired_res`` ``<-`` `[`compare_omic_signatures`](https://montilab.github.io/OmicSignature/reference/compare_omic_signatures.md)`(`\
...
\
`toy_ks_score`` ``<-`` `[`compare_omic_signatures`](https://montilab.github.io/OmicSignature/reference/compare_omic_signatures.md)`(`\
`  sig_list1 ``=`` ``compare_label_pairing_example``,`\
`  method ``=`` ``"ks_score"``,`\
`  label_pairing ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`\
`    signature_a ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"treated"``, ``"control"``)``,`\
...
`)`\
\
[`round`](https://rdrr.io/r/base/Round.html)`(``toy_ks_score``$``comparisons``$``level1_vs_level1``$``score``, ``3``)`\
`#>             signature_a signature_b signature_c`\
`#> signature_a         1.0         0.8       0.556`\
`#> signature_b         0.8         1.0       0.556`\
`#> signature_c         0.5         0.5       1.000`\
\
[`signif`](https://rdrr.io/r/base/Round.html)`(``toy_ks_score``$``comparisons``$``level1_vs_level1``$``pvalue``, ``3``)`\
`#>             signature_a signature_b signature_c`\
`#> signature_a    5.78e-14    2.83e-10    6.24e-04`\
`#> signature_b    2.83e-10    5.78e-14    6.24e-04`\
`#> signature_c    1.03e-03    3.44e-05    5.78e-14`\
\
[`round`](https://rdrr.io/r/base/Round.html)`(``toy_ks_score``$``comparisons``$``level2_vs_level2``$``score``, ``3``)`\
`#>             signature_a signature_b signature_c`\
`#> signature_a         1.0         0.8         0.5`\
`#> signature_b         0.8         1.0         0.5`\
`#> signature_c         0.5         0.5         1.0`\
\
[`signif`](https://rdrr.io/r/base/Round.html)`(``toy_ks_score``$``comparisons``$``level2_vs_level2``$``pvalue``, ``3``)`\
`#>             signature_a signature_b signature_c`\
...
\
`gsea_res`` ``<-`` `[`compare_omic_signatures`](https://montilab.github.io/OmicSignature/reference/compare_omic_signatures.md)`(`\
`  sig_list1 ``=`` ``signature_list``,`\
`  method ``=`` ``"gsea"``,`\
`  adj_p_cutoff ``=`` ``0.05``,`\
`  min_features ``=`` ``25``,`\
...
`  mode ``=`` ``"separate"``,`\
`  triangle ``=`` ``"upper"``,`\
`  column_names_gp ``=`` ``grid``::`[`gpar`](https://rdrr.io/r/grid/gpar.html)`(``fontsize ``=`` ``9``)``,`\
`  row_names_gp ``=`` ``grid``::`[`gpar`](https://rdrr.io/r/grid/gpar.html)`(``fontsize ``=`` ``9``)``,`\
`  column_names_rot ``=`` ``45`\
`)`
