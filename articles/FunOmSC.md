...
\
`OmS1`` ``<-`` `[`readJson`](https://montilab.github.io/OmicSignature/reference/readJson.md)`(`[`file.path`](https://rdrr.io/r/base/file.path.html)`(`[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, package ``=`` ``"OmicSignature"``)``, ``"OmS_example_1.json"``)``)`\
`#>   [Success] OmicSignature object Experiment in liver created.`\
`OmS2`` ``<-`` `[`readJson`](https://montilab.github.io/OmicSignature/reference/readJson.md)`(`[`file.path`](https://rdrr.io/r/base/file.path.html)`(`[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, package ``=`` ``"OmicSignature"``)``, ``"OmS_example_2.json"``)``)`\
`#>   [Success] OmicSignature object Experiment in brain created.`\
`OmS3`` ``<-`` `[`readJson`](https://montilab.github.io/OmicSignature/reference/readJson.md)`(`[`file.path`](https://rdrr.io/r/base/file.path.html)`(`[`system.file`](https://rdrr.io/r/base/system.file.html)`(``"extdata"``, package ``=`` ``"OmicSignature"``)``, ``"OmS_example_3.json"``)``)`\
...
`  ``"author"`` ``=`` ``"vmli"`\
`)`\
`OmSC`` ``<-`` `[`OmicSignatureCollection`](https://montilab.github.io/OmicSignature/reference/OmicSignatureCollection.md)`$``new``(`\
`  OmicSigList ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``OmS1``, ``OmS2``, ``OmS3``)``,`\
`  metadata ``=`` ``ColMeta``,`\
`  print_message ``=`` ``FALSE`\
`)`\
