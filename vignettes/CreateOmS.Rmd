...
- **signature**, a data frame. \
  Required columns: \
    ...
    "**feature_symbol**" (e.g. ENSEMBL ID, Uniprot ID) \
...
- **difexp** (optional), a data frame of differential expression analysis results. \
  ...
  Recommended optional column, if applicable: "score", "feature_symbol" \
...
"**probe_id**", "**feature_symbol**", along with at least one of the following: "**p_value**", "**q_value**", or "**adj_p**". \
...
"**feature_symbol**" is a name that identifies each feature, examples include ENSEMBL IDs, UniProt IDs, and Refmet IDs. To better identify the features, it is recommended to add an additional annotation column (e.g., "gene_name") to specify a more human readable name (e.g., CD248). For metabolite features, it is recommended to include multiple annotation columns is available (e.g. HMDB ID, InChI key). \
...
### 2.3. Signature 
**signature** is a dataframe with column **"probe_id"** and **"feature_symbol"**. If the signature is "bi-directional" or "categorical" (specified in `direction_type` in `metadata`), then column **"direction"** is also required. \
...
```
signature <- data.frame("probe_id" = c(1, 2, 3), "feature_symbol" = c("gene1", "gene2", "gene3"))
```
...
signature <- data.frame(
  "probe_id" = c(1, 2, 3),
  "feature_symbol" = c("gene1", "gene2", "gene3"),
...
signature <- data.frame(
  "probe_id" = c(1, 2, 3, 4),
  "feature_symbol" = c("gene1", "gene2", "gene3", "gene4"),
