...
## 1. Enssentials to create an OmicSignature Object 
An `OmicSignature` object contains three parts:  
 - **metadata**, a list contains metadata fields.  
  the required fields are: **"signature_name", "organism", "platform", "direction_type", "phenotype"**.  
 - **signature**, a dataframe with columns:  
  **"signature_symbol"**, **"signature_direction"**, and **"signature_score"** (optional).  
 - **difexp** (optional) a dataframe of differential expression analysis result.  
  the required columns are **"id", "symbol", "score", "adj_p"**.  
...
Or, you can **read an `OmicSignature` saved in .json format using `readJson()`.** 
See in ["Functionalities, imports and exports"](https://montilab.github.io/OmicSignature/articles/FunOmS.html) section. 
...
Or use build-in function `createMetadata()` (recommended). The function can help remind you what attributes to include. 
```{r}
...
#### 2.1.2 Add more info in metadata
