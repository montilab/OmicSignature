...
**signature** is a dataframe with columns **"probe_id"** and **"feature_name"**. \
If the signature is "bi-directional" or "categorical" (specified in `direction_type` in `metadata`), then column **"group_label"** is also required. This column indicates in which phenotype a feature is more highly expressed or more significant. \
For example, if the analysis is identifying genes differentially expressed in Treatment vs Control, all gene features with a positive score should be labeled "Treatment" in this column to show their higher expression in the Treatment group. Features with negative scores would instead be labeled "Control". Alternatively, if the analysis is about protein expression changes associated with BMI, protein features with a positive (or negative) score should be labeled "higher BMI" (or "lower BMI"). \
 \
