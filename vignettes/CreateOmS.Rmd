...
- **signature**, a dataframe. \
  Required columns: "**id**", "**symbol**", "**direction**" \
...
"**platform**", "**sample_type**", "**covariates**", "**description**",
"**adj_p_cutoff**", "**score_cutoff**". \

Usually, "phenotype" is a one or two-word description of a topic, such as a drug treatment, a gene knock out, or a research area, for example, longevity or breast cancer. \
...
- "multiple". Used with multi-valued categorical phenotypes (e.g., "A" _vs._ "B" _vs._ "C"), usually analyzed by ANOVA. \

#### 2.1.3 "assay_type"
`assay_type` is one of the following: \
- "transcriptomics" (e.g. gene expression, RNA-seq) \
- "proteomics" \
- "metabolomics" \
- "methylomics" \
...
Some frequently used optional column examples: \
"logfc", "beta", "mean", "se", "robust_se", "HR", "adj_p", "annotation". \
...
### 2.3. Signature 
**signature** is a dataframe with column **"id"** and **"symbol"**, which should be subsets of "id" and "symbol" in the difexp, if present. If the signature is "bi-directional" or "multiple" (specified in `direction_type` in `metadata`), then column **"direction"** is also required. \
